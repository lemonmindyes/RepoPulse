...
def _extract_link_count(tree: etree._Element, href_fragment: str) -> str:
    anchors = tree.xpath(
        f'//nav[@aria-label="Repository"]//a[contains(@href, "{href_fragment}")]'
    )
    if not anchors:
        anchors = tree.xpath(f'//a[contains(@href, "{href_fragment}")]')
...
async def get_repo_detail_info(
    ...
    async with semaphore:
        async with session.get(
            repo_info["repo_url"], proxy="http://127.0.0.1:7890"
        ) as response:
            html = await response.text()
    tree = etree.HTML(html)
    ...
    # 1、获取issue数
    repo_issue = _extract_link_count(tree, f"{repo_path}/issues")
    repo_info["repo_issue"] = repo_issue

    # 2、获取Pr数
    repo_pr = _extract_link_count(tree, f"{repo_path}/pulls")
    repo_info["repo_pr"] = repo_pr

    # 3、获取commit数
    repo_commit = _extract_link_count(tree, f"{repo_path}/commits")
    if repo_commit == "0":
    ...
    # 5、获取README.md
    repo_readme = tree.xpath(
        '//article[@class="markdown-body entry-content container-lg"]//text()'
    )
...
async def get_trending_async(
    ...
    headers = {"User-Agent": UserAgent().edge, "Cookie": Cookie}
    ssl_context = ssl.create_default_context(cafile=certifi.where())
    connector = aiohttp.TCPConnector(
        ssl=ssl_context, family=socket.AF_INET, happy_eyeballs_delay=None
    )
    ...
    async with aiohttp.ClientSession(headers=headers, connector=connector) as session:
        ...
        # 2、获取所有仓库的详细信息
        tasks = [
            get_repo_detail_info(session, repo_info, semaphore)
